...
Foreach ($Target in $Targets){
	...
	<font face="Arial" size="1"><b><i>Dev. Version 0.1 by Jon Biddell</i></b></font><br>
	...
	Foreach ($objService in $colListOfServices)
	{
		...
		If ($objService.StartMode -eq "Auto")
		{
			if ($objService.State -eq "Stopped")
			{
				...
				$Report+=  "						<td width='25%'><font face='Wingdings'color='#FF0000'>Ã»</font></td>"
			}
		}
		If ($objService.StartMode -eq "Auto")
		{
			if ($objService.State -eq "Running")
			{
				...
				$Report+=  "						<td width='20%'><font face='Wingdings'color='#009900'>Ã¼</font></td>"
			}
		}
